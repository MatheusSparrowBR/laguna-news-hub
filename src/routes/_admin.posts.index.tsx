...
import { useAlterarStatusPost, useCampanhas, usePatrocinadores, usePostsProjeto, useSalvarPost } from "@/services/editorialQueries";
...
export const Route = createFileRoute("/_admin/posts/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Monte publicações de notícia ou patrocinadas com título, legenda, hashtags e arte prontos." },
      { property: "og:title", content: "Publicações | HORA NEWS LAGUNA" },
      { property: "og:description", content: "Compositor de publicações do perfil de notícias de Laguna." },
...
function PostsPage() {
  ...
  return (
    <PageContainer
      ...
      acoes={<Button size="sm" onClick={abrirNovo}><Plus className="size-4" />Nova publicação</Button>}
    >
      {isLoading ? (
        <LoadingState titulo="Carregando publicações..." />
      ) : (posts ?? []).length === 0 ? (
        <EmptyState titulo="Nenhuma publicação criada" descricao="Crie a primeira a partir de uma notícia aprovada ou de uma campanha." />
      ) : (
            ...
            <Card key={post.id}>
              <CardContent className="space-y-2 pt-4">
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant={post.is_sponsored ? "outline" : "secondary"}>{post.is_sponsored ? "PUBLICIDADE" : "Notícia"}</Badge>
                  <Badge variant="outline">{STATUS_POST_ROTULO[post.status] ?? post.status}</Badge>
                  {post.scheduled_at ? <span className="text-xs text-muted-foreground">agendada para {formatarDataHora(post.scheduled_at)}</span> : null}
                </div>
                <h2 className="text-sm font-semibold leading-snug">{post.title ?? "(sem título)"}</h2>
                {post.caption ? <p className="line-clamp-2 text-xs text-muted-foreground">{post.caption}</p> : null}
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" variant="outline" onClick={() => { setEmEdicao(post); setCompositorAberto(true); }}>
                    <PenLine className="size-4" />Editar
                  </Button>
                  {post.status !== "cancelled" && post.status !== "published" ? (
                    <Button size="sm" variant="ghost" disabled={alterarStatus.isPending} onClick={() => alterarStatus.mutate({ id: post.id, status: "cancelled" })}>Cancelar</Button>
                  ) : null}
                </div>
              </CardContent>
            </Card>
