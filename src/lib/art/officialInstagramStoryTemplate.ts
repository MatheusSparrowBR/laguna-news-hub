...
export interface OfficialInstagramStoryArtInput {
...
export const OFFICIAL_INSTAGRAM_STORY_TEMPLATE = {
  key: "hora-news-laguna-official-story",
  format: "story" as const,
  width: DIMENSOES.story.width,
  height: DIMENSOES.story.height,
  logoPath: "/branding/hora-news-laguna-logo.png",
} as const;

const FONT = "Inter, Arial, Helvetica, sans-serif";
const W = DIMENSOES.story.width;
const H = DIMENSOES.story.height;
...
export function renderOfficialInstagramStorySvg(input: OfficialInstagramStoryArtInput): string {
  ...
  const summary = input.summary
    ...
    : null;
  const date = esc(formatDate(input.dateLabel));
  ...
  parts.push(`<defs><clipPath id="story-photo"><rect x="0" y="0" width="${W}" height="1110"/></clipPath><linearGradient id="story-overlay" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${MARCA.primary}" stop-opacity=".05"/><stop offset="72%" stop-color="${MARCA.primary}" stop-opacity=".15"/><stop offset="100%" stop-color="${MARCA.primary}" stop-opacity=".94"/></linearGradient></defs>`);
  parts.push(`<rect width="${W}" height="${H}" fill="${MARCA.primary}"/>`);
  if (input.imageUrl) {
    parts.push(`<image href="${esc(input.imageUrl)}" x="0" y="0" width="${W}" height="1110" preserveAspectRatio="xMidYMid slice" clip-path="url(#story-photo)"/>`);
    parts.push(`<rect width="${W}" height="1110" fill="url(#story-overlay)"/>`);
  } else {
    parts.push(`<rect width="${W}" height="1110" fill="${MARCA.secondary}" opacity=".55"/>`);
  }
  ...
  parts.push(`<rect x="${safe}" y="1175" width="8" height="${Math.max(130, Math.min(250, title.linhas.length * title.fontSize * title.lineHeight + 30))}" rx="4" fill="${MARCA.accent}"/>`);

  const titleY = 1230;
  title.linhas.forEach((line, index) => {
    const lastLine = index === title.linhas.length - 1 && title.linhas.length > 1;
    parts.push(`<text x="${safe + 26}" y="${titleY + index * title.fontSize * title.lineHeight}" font-family="${FONT}" font-size="${title.fontSize}" font-weight="900" fill="${lastLine ? MARCA.accent : MARCA.white}">${esc(line)}</text>`);
  });

  let cursorY = titleY + title.linhas.length * title.fontSize * title.lineHeight + 26;
  if (summary) {
    for (const line of summary.linhas) {
      parts.push(`<text x="${safe + 26}" y="${cursorY}" font-family="${FONT}" font-size="${summary.fontSize}" font-weight="500" fill="${MARCA.white}" opacity=".96">${esc(line)}</text>`);
  ...
  parts.push(`<text x="${safe}" y="${footerY + 10}" font-family="${FONT}" font-size="20" font-weight="700" fill="${MARCA.white}">●  ${location}</text>`);
  parts.push(`<text x="${safe}" y="${footerY + 52}" font-family="${FONT}" font-size="18" font-weight="600" fill="${MARCA.white}">Foto: ${credit}</text>`);
  parts.push(`<image href="${OFFICIAL_INSTAGRAM_STORY_TEMPLATE.logoPath}" x="770" y="1670" width="250" height="250" preserveAspectRatio="xMidYMid meet"/>`);
  parts.push(`<path d="M0 1840 C250 1805 430 1870 690 1830 C850 1805 950 1830 1080 1800 L1080 1920 L0 1920Z" fill="${MARCA.secondary}" opacity=".95"/>`);
  parts.push(`<path d="M0 1870 C260 1835 470 1895 710 1860 C880 1835 970 1860 1080 1830" fill="none" stroke="${MARCA.accent}" stroke-width="8"/>`);
