 ...
 * A fotografia da notícia é a base visual do post inteiro, com overlays da marca
 * para manter legibilidade. A assinatura permanece fixa no rodapé.
 */
...
export function renderOfficialInstagramSvg(input: OfficialInstagramArtInput): string {
  ...
  parts.push(`
    <svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
      <defs>
        <clipPath id="official-photo"><rect width="${W}" height="${H}" /></clipPath>
        <clipPath id="branding-clip">
        ...
        <linearGradient id="official-overlay" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="${MARCA.primary}" stop-opacity=".03" />
          <stop offset="48%" stop-color="${MARCA.primary}" stop-opacity=".06" />
          <stop offset="72%" stop-color="${MARCA.primary}" stop-opacity=".34" />
          <stop offset="100%" stop-color="${MARCA.primary}" stop-opacity=".82" />
        </linearGradient>
      </defs>`);

  if (input.imageUrl) {
    // A foto ocupa todo o 1080×1350 para que não exista um fundo chapado
    // independente da notícia. O overlay preserva a identidade e o contraste.
    parts.push(`<image href="${esc(input.imageUrl)}" x="0" y="0" width="${W}" height="${H}" preserveAspectRatio="xMidYMid slice" clip-path="url(#official-photo)"/>`);
    parts.push(`<rect width="${W}" height="${H}" fill="url(#official-overlay)"/>`);
  } else {
  ...
  // Painel editorial translúcido. A foto continua visível atrás do texto.
  parts.push(`<path d="M0 ${panelTop} C180 ${panelTop - 18} 350 ${panelTop + 22} 540 ${panelTop - 2} C730 ${panelTop - 28} 900 ${panelTop + 4} 1080 ${panelTop - 10} L1080 1350 L0 1350Z" fill="${MARCA.primary}" opacity=".70"/>`);
  parts.push(`<path d="M0 ${panelTop + 2} C220 ${panelTop - 18} 370 ${panelTop + 35} 570 ${panelTop + 4} C770 ${panelTop - 22} 900 ${panelTop + 20} 1080 ${panelTop - 8} L1080 ${panelTop + 55} L0 ${panelTop + 55}Z" fill="${MARCA.primary}" opacity=".34"/>`);
  ...
  // Metadados editoriais ficam acima do rodapé de marca e usam o crédito informado.
  const footerY = 1140;
  ...
  // Ondas inferiores ficam atrás da assinatura.
  parts.push(`<path d="M0 1288 C230 1250 380 1320 610 1282 C790 1250 920 1270 1080 1238 L1080 1350 L0 1350Z" fill="${MARCA.secondary}" opacity=".70"/><path d="M0 1315 C250 1280 430 1342 680 1305 C850 1280 950 1300 1080 1270" fill="none" stroke="${MARCA.accent}" stroke-width="7"/>`);

  // Faixa de assinatura fixa, agora com o mesmo fundo fotográfico da notícia.
  if (input.imageUrl) {
    parts.push(`<image href="${esc(input.imageUrl)}" x="0" y="0" width="${W}" height="${H}" preserveAspectRatio="xMidYMid slice" clip-path="url(#branding-clip)" opacity=".82"/>`);
    parts.push(`<rect x="${OFFICIAL_INSTAGRAM_TEMPLATE.brandingBarX}" y="${OFFICIAL_INSTAGRAM_TEMPLATE.brandingBarY}" width="${OFFICIAL_INSTAGRAM_TEMPLATE.brandingBarWidth}" height="${OFFICIAL_INSTAGRAM_TEMPLATE.brandingBarHeight}" rx="12" fill="${MARCA.primary}" opacity=".48"/>`);
  } else {
