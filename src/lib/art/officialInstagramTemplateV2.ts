...
export function renderOfficialInstagramSvg(
  ...
  const photoBottom = height * (isFeed ? 0.64 : 0.58);
  ...
  parts.push(`<defs>`);
  parts.push(
    `<clipPath id="${photoClipId}"><rect x="0" y="0" width="${width}" height="${photoBottom + 55 * scale}"/></clipPath>`,
  );
  parts.push(
    `<linearGradient id="${overlayId}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${MARCA.primary}" stop-opacity="0"/><stop offset="70%" stop-color="${MARCA.primary}" stop-opacity="0.04"/><stop offset="100%" stop-color="${MARCA.primary}" stop-opacity="0.86"/></linearGradient>`,
  );
  parts.push(`</defs>`);
  ...
  if (input.imageUrl) {
    parts.push(
      `<image href="${esc(input.imageUrl)}" x="0" y="0" width="${width}" height="${photoBottom + 65 * scale}" preserveAspectRatio="xMidYMid slice" clip-path="url(#${photoClipId})"/>`,
    );
    parts.push(
      `<rect x="0" y="0" width="${width}" height="${photoBottom + 65 * scale}" fill="url(#${overlayId})"/>`,
    );
  } else {
    parts.push(
      `<rect x="0" y="0" width="${width}" height="${photoBottom + 65 * scale}" fill="${MARCA.secondary}" opacity="0.45"/>`,
    );
  }
  ...
  parts.push(
    `<rect x="${pillX}" y="${pillY}" width="${pillW}" height="${pillH}" rx="12" fill="${theme.badge === MARCA.accent ? MARCA.accent : MARCA.primary}"/>`,
  );
  parts.push(
    `<circle cx="${pillX + 29 * scale}" cy="${pillY + pillH / 2}" r="16" fill="${MARCA.accent}"/>`,
  );
  parts.push(
    `<text x="${pillX + 55 * scale}" y="${pillY + 36 * scale}" font-family="${FONT}" font-size="${25 * scale}" font-weight="900" fill="${theme.badge === MARCA.accent ? MARCA.text : MARCA.white}">${label}</text>`,
  );

  const dateW = 270 * scale;
  const dateX = width - safe * 0.65 - dateW;
  parts.push(
    `<rect x="${dateX}" y="${pillY}" width="${dateW}" height="${pillH}" rx="12" fill="${MARCA.accent}"/>`,
  );
  parts.push(
  ...
  parts.push(
    `<rect x="${safe}" y="${titleY - 28 * scale}" width="${7 * scale}" height="${Math.max(95 * scale, Math.min(140 * scale, estimatedTitleHeight + 25 * scale))}" rx="3" fill="${MARCA.accent}"/>`,
  );

  for (let i = 0; i < title.linhas.length; i += 1) {
    const useAccent = i === title.linhas.length - 1 && title.linhas.length > 1;
    parts.push(
      `<text x="${safe + 24 * scale}" y="${titleY + i * titleLineHeight}" font-family="${FONT}" font-size="${title.fontSize}" font-weight="900" fill="${useAccent ? MARCA.accent : MARCA.white}">${esc(title.linhas[i])}</text>`,
    );
  }
  ...
  if (summary) {
    for (const line of summary.linhas) {
      parts.push(
        `<text x="${safe + 24 * scale}" y="${cursorY}" font-family="${FONT}" font-size="${summary.fontSize}" font-weight="500" fill="${MARCA.white}" opacity="0.97">${esc(line)}</text>`,
      );
  ...
  parts.push(
    `<text x="${safe}" y="${footerY + 8 * scale}" font-family="${FONT}" font-size="${20 * scale}" font-weight="700" fill="${MARCA.white}">●  ${location}</text>`,
  );
  parts.push(
    `<text x="${safe + 300 * scale}" y="${footerY + 8 * scale}" font-family="${FONT}" font-size="${19 * scale}" font-weight="600" fill="${MARCA.white}">│  Foto: ${credit}</text>`,
  );
  ...
  parts.push(
    `<path d="M0 ${height - 31 * scale} Q ${width * 0.38} ${height - 7 * scale}, ${width * 0.7} ${height - 40 * scale} T ${width} ${height - 22 * scale}" fill="none" stroke="${MARCA.accent}" stroke-width="${7 * scale}"/>`,
  );

  if (input.sponsorName) {
    parts.push(
      `<text x="${safe}" y="${height - 88 * scale}" font-family="${FONT}" font-size="${18 * scale}" font-weight="800" fill="${MARCA.text}">PUBLICIDADE • ${esc(input.sponsorName)}</text>`,
    );
  }
