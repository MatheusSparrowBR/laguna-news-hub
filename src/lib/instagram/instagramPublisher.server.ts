...
export async function getAccount(supabase: Cliente, projectId: string): Promise<ContaInstagram | null> {
  const { data } = await supabase
    .from("social_accounts")
    .select("id, provider, account_id, username, display_name, status, scopes, connected_at, last_verified_at, token_expires_at")
...
export async function validateConnection(supabase: Cliente, projectId: string): Promise<EstadoConexao> {
  ...
  return { conectado: !!conta && conta.status === "connected", conta, config, pendencias };
}
...
function falha(codigo: string, statusHttp?: number): ResultadoPublicacao {
  ...
  return { ok: false, state: "failed", externalId: null, errorCode: amigavel.codigo, errorMessage: amigavel.mensagem };
}

async function contexto(supabase: Cliente, projectId: string): Promise<{ ok: true; igUserId: string; accessToken: string } | { ok: false; resultado: ResultadoPublicacao }> {
  const estado = await validateConnection(supabase, projectId);
  if (!estado.conectado || !estado.conta?.account_id) return { ok: false, resultado: falha("not_connected") };
  ...
  if (credencial.expiresAt && new Date(credencial.expiresAt).getTime() < Date.now()) return { ok: false, resultado: falha("token_expired") };
  return { ok: true, igUserId: estado.conta.account_id, accessToken: credencial.accessToken };
}

export async function createMediaContainer(
  supabase: Cliente,
  entrada: { projectId: string; asset: AssetParaPublicacao; caption: string; kind?: MediaKind; idempotencyKey: string },
  ...
  const resposta = await fetch(url, {
    ...
    body: JSON.stringify({ image_url: entrada.asset.publicUrl, caption: entrada.caption }),
  });
  if (!resposta.ok) {
...
export async function checkMediaStatus(supabase: Cliente, projectId: string, containerId: string): Promise<ResultadoPublicacao> {
  ...
  if (json.status_code === "FINISHED") return { ok: true, state: "queued", externalId: containerId, errorCode: null, errorMessage: null };
  if (json.status_code === "ERROR") return falha("media_rejected");
  return falha("publish_in_progress");
}

export async function publishMedia(supabase: Cliente, entrada: { projectId: string; postId: string; containerId: string }): Promise<ResultadoPublicacao> {
  const ctx = await contexto(supabase, entrada.projectId);
  if (!ctx.ok) return ctx.resultado;
...
export async function getPublishedMedia(supabase: Cliente, projectId: string): Promise<{ conectado: boolean; itens: never[] }> {
  const estado = await validateConnection(supabase, projectId);
  return { conectado: estado.conectado, itens: [] };
}

/** Publicação manual com log persistido exclusivamente pelo backend. */
export async function publicarPostAgora(
  supabase: Cliente,
  entrada: { projectId: string; postId: string; asset: AssetParaPublicacao; caption: string; idempotencyKey: string; tentativa?: number },
  ...
  if (jaPublicado) {
    return {
      resultado: { ok: true, state: "published", externalId: jaPublicado.external_id, errorCode: null, errorMessage: null },
  ...
  if (container.ok && container.externalId) {
    resultado = await publishMedia(supabase, { projectId: entrada.projectId, postId: entrada.postId, containerId: container.externalId });
  }

  const publicado = resultado.ok && resultado.state === "published";
  const admin = criarClienteAdmin();
  const { error: logError } = await admin.from("publication_logs").insert({
  ...
  if (logError) console.error("[instagram] falha ao registrar publication_log", logError.message);
