...
export function ArtPreview({ entrada, formatos = ["feed"] }: { entrada: Omit<EntradaArte, "format">; formatos?: readonly ArtFormat[] }) {
  return (
    <div className="flex flex-wrap gap-4">
      {formatos.map((formato) => {
        ...
        return <figure key={formato} className="space-y-2">
          <img src={svgParaDataUrl(svg)} alt={`Arte ${ROTULO_FORMATO[formato]}: ${entrada.title}`} width={largura} height={Math.round((largura * height) / width)} className="rounded-lg border border-border shadow-sm" />
